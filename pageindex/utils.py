...
# Backward-compat env vars (still read for fallback)
CHATGPT_API_KEY = os.getenv("API_KEY")
BASE_URL = os.getenv("BASE_URL")

# ---------------------------------------------------------------------------
# LLMProvider-backed wrapper: lazily import to avoid circular deps
# ---------------------------------------------------------------------------


def _get_default_provider(model=None):
    """Lazily get the default LLMProvider instance.

    If *model* is provided, it overrides the provider's default model
    so that callers (page_index_md, page_index) can pass the model
    they received from the user.
    """
    from ldrs.llm_provider import get_provider

    return get_provider(model_override=model)
...
def ChatGPT_API_with_finish_reason(
    model, prompt, api_key=CHATGPT_API_KEY, chat_history=None, llm_provider=None
):
    max_retries = 10
    provider = llm_provider or _get_default_provider(model=model)
...
async def ChatGPT_API_async(model, prompt, api_key=CHATGPT_API_KEY, llm_provider=None):
    max_retries = 10
    messages = [{"role": "user", "content": prompt}]
    provider = llm_provider or _get_default_provider(model=model)
    for i in range(max_retries):
